...
export const Header = () => {
  ...
  const navLinks: NavLink[] = [
    { name: "About", type: "anchor", href: "#hero" },
  ...
  const scrollToSection = (e: React.MouseEvent<HTMLAnchorElement>, href: string) => {
    ...
    if (location.pathname !== "/") {
      // Sections only exist on the home page - go there first, then scroll
  ...
  return (
    ...
    >
      <div className="container mx-auto px-6 flex justify-between items-center">
        ...
        <div className="md:hidden flex items-center gap-3">
          <button
            onClick={toggleDarkMode}
            aria-label={darkMode ? "Switch to light mode" : "Switch to dark mode"}
            className="p-2 rounded-lg bg-gray-100 dark:bg-white/5 border dark:border-white/10"
          >
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`md:hidden absolute top-[100%] left-0 w-full overflow-hidden transition-all duration-300 ease-in-out ${
